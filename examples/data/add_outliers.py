...
# https://github.com/OpenSLAM-org/openslam_vertigo/blob/master/datasets/generateDataset.py
...
def checkOptions(options):
    ...
    if options.filename == "" or options.filename is None:
    ...
    if options.information:
        if (
            (options.information.count(",") != 0)
            and (options.information.count(",") != 5)
            and (options.information.count(",") != 20)
        ):
            print(
                "Information matrix must be given in full upper-triangular form. E.g. --information=42,0,0,42,0,42 or as a single value that is used for all diagonal entries, e.g. --information=42."
            )
            return False
...
def writeDataset(
    ...
    groupSize=1,
    doLocal=0,
    informationMatrix="42,0,0,42,0,42",
    doSwitchable=True,
    doMaxMix=False,
    doMaxMixAgarwal=False,
    perfectMatch=False,
):
        ...
        if not informationMatrix:
            ...
            if informationMatrix.count(",") == 0:
                ...
                try:
                    ...
                    print(
                        "! Invalid value for information matrix. If you give only a single value, it must be a number, e.g. --information=42"
                    )
                    return False
                ...
                print(
                    "! Invalid number of entries in information matrix. Full upper triangular form has to be given, e.g. --information=42,0,0,42,0,42."
                )
                return False
        ...
        if not informationMatrix:
            ...
            if informationMatrix.count(",") == 0:
                ...
                try:
                    ...
                    return False
                informationMatrix = (
                    "%f,0,0,0,0,0,%f,0,0,0,0,%f,0,0,0,%f,0,0,%f,0,%f"
                    % (diagEntry, diagEntry, diagEntry, diagEntry, diagEntry, diagEntry)
                )
            elif informationMatrix.count(",") != 20:
                print(
                    "! Invalid number of entries in information matrix (--information). Full upper triangular form has to be given."
                )
                return False
    else:
    ...
    for oldStr in edges:
        ...
        if not isOdometryEdge and informationMatrix is None:
            elem = oldStr.split()
            if mode == 2:
        ...
        if doSwitchable and not isOdometryEdge:
            s = " ".join(
                ["VERTEX_SWITCH", str(poseCount + switchCount), str(switchPrior)]
            )
            f.write(s + "\n")
            ...
            elem = oldStr.split()
            s = " ".join(
                [edgeStr + "_MAXMIX"] + [elem[1], elem[2], str(maxmixScale)] + elem[3:]
            )
            f.write(s + "\n")
            ...
            edge2 = " ".join(
                [edgeStr, str(maxmixWeight)] + elem[1:-nInfo] + weighted_info_str
            )
    ...
    for i in range(outliers):
        ...
        for j in range(groupSize):
            ...
            if doSwitchable:
                ...
                s = (
                    " ".join([edgeStr + "_SWITCHABLE"] + [str(x) for x in n])
                    + " "
                    + info_str
                )
                f.write(s + "\n")
                ...
                s = (
                    " ".join([edgeStr + "_MAXMIX"] + [str(x) for x in n])
                    + " "
                    + info_str
                )
                f.write(s + "\n")
                ...
                # put it together
                s = (
                    " ".join([edgeStr + "_MIXTURE", str(v1), str(v2), "2"])
                    + " "
                    + edge1
                    + " "
                    + edge2
                ...
                if mode == 3:
                    n.extend([q0, q1, q2, q3])
                    s = (
                        " ".join([edgeStr + ":QUAT"] + [str(x) for x in n])
                        + " "
                        + info_str
                    )
                else:
...
if __name__ == "__main__":
    ...
    parser.add_option(
        "-o", "--out", help="Results will be written into this file.", default=None
    )
    parser.add_option(
        "-n",
        "--outliers",
        help="Spoil the dataset with this many outliers. Default = 100.",
        default=100,
        type="int",
    )
    parser.add_option(
        "-g",
        "--groupsize",
        help="Use this groupsize. Default = 1.",
        default=1,
        type="int",
    )
    parser.add_option(
        "--switchCov",
        help="Set the switch covariance matrix. Default = 1.0",
        default=1.0,
        type="float",
    )
    parser.add_option(
        "--information",
        help="Set the information matrix for the additional false positive loop closure constraints. Provide either a single value e.g. --information=42 that will be used for all diagonal entries. Or provide the full upper triangular matrix using values separated by commas, but no spaces: --information=42,0,0,42,0,42 etc.",
    )  # , default="42.7,0,0,42.7,0,42.7")
    parser.add_option(
        "--seed",
        help="Random seed. If >0 it will be used to initialize the random number generator to create repeatable random false positive loop closures.",
        default=0,
        type="int",
    )
    parser.add_option(
        "--maxmixWeight",
        help="Weight factor for the null hypothesis used in the max-mixture model. Default = 0.01",
        default=0.01,
        type="float",
    )
    parser.add_option(
        "--maxmixScale",
        help="Scale factor for the null hypothesis used in the max-mixture model. Default = 10e-12",
        default=10e-12,
        type="float",
    )
    ...
    parser.add_option(
        "-s",
        "--switchable",
        help="Use the switchable loop closure constraints.",
        action="store_true",
        default=False,
    )
    parser.add_option(
        "-m",
        "--maxmix",
        help="Use the max-mixture loop closure constraints.",
        action="store_true",
        default=False,
    )
    parser.add_option(
        "--maxmixAgarwal",
        help="Use the max-mixture loop closure constraints but create a dataset file that is compatible to the format of Pratik Agarwal's original Max-Mixture code.",
        action="store_true",
        default=False,
    )
    parser.add_option(
        "-l",
        "--local",
        help="Create only local false positive loop closure constraints.",
        action="store_true",
        default=False,
    )
    parser.add_option(
        "-p",
        "--perfectMatch",
        help="Loop closures match perfectly, i.e. the transformation between both poses is (0,0,0).",
        action="store_true",
        default=False,
    )
    ...
    if options.out is None:
        options.out = options.filename + ".outliers"
